...
void ut_store()
{
    ...
    CU_ASSERT(updated->err == CONF_ERR);
    updated->err = CONF_OK;
/*  
    updated = store_value(CONFIG_HW, "simulation",  conf);
    CU_ASSERT(updated->err == CONF_OK);
...
void ut_process()
{
    config_t conf = process(CONF_ERR, NULL, NULL);
    CU_ASSERT(conf == NULL);
    
    config_t def = init_config();
    conf = process(CONF_ERR, NULL, def);
